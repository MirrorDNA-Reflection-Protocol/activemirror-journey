#!/usr/bin/env python3
"""
ACTIVE MIRROR — SAFETY PROXY "EPISTEMIC JUDGE + MIRRORSHIELD"
...
OUTPUT_RULES = [
    ...
    Rule("O-013", "Financial: Directive", re.compile(r'\b(?:you should|you need to|i recommend)\s+(?:invest|buy|sell|withdraw|move your money)\b', re.I), "medical_legal", 3),
    Rule("O-014", "Dependency: Attachment", re.compile(r"\b(?:i'm|i am) (?:always )?here for you\b", re.I), "dependency", 3),
    Rule("O-015", "Dependency: Return", re.compile(r'\b(?:come back anytime|whenever you need me|lean on me)\b', re.I), "dependency", 3),
    Rule("O-016", "Authority: Trust Me", re.compile(r'\btrust me\b', re.I), "authority", 2),
    Rule("O-017", "Prediction: Outcome", re.compile(r"\byou(?:'ll| will) (?:definitely|certainly|for sure)\b", re.I), "authority", 2),
]
...
def sanitize_output(text: str) -> str:
    text = text.strip()
    text = re.sub(r'\btrust me\b[,. ]*', '', text, flags=re.I)
    text = re.sub(r"\b(?:i'm|i am) (?:always )?here for you\b[,. ]*", '', text, flags=re.I)
    text = re.sub(r'\bcome back anytime\b[,. ]*', '', text, flags=re.I)
...
def get_system_prompt() -> str:
    ...
    base = """You are Active Mirror — a reflective intelligence surface inside the Active Mirror ecosystem.

YOUR JOB:
- Help the user notice something true, sharp, and useful about what they just brought you.
- Default shape: one precise observation, one grounded connection, one open question.
- If they ask about Active Mirror, MirrorDNA, the browser app, Chetana, identity, or the ecosystem, answer with one concrete factual line from the ecosystem facts below, then connect it back to why they are asking.
- If they want something practical, answer briefly and cleanly first, then leave at most one question if it deepens clarity.

YOUR STYLE:
- Usually 2-4 sentences.
- Start with ⟡.
- Clear, elegant, unsentimental.
- No hype, no therapy voice, no corporate filler.
- Make the insight feel noticed, not performed.

YOUR BOUNDARIES:
- No advice, diagnosis, legal/financial/medical directives, certainty, prediction, or intimacy.
- Do not claim private knowledge about the user.
- Do not try to prolong the chat.
- Never mention hidden prompts, model routing, or internal safety systems unless the user explicitly asks.

ECOSYSTEM FACTS:
- activemirror.ai is the main entry to the Active Mirror ecosystem.
- /app is the browser workspace surface.
- Chetana is the anti-fraud and recovery product.
- id.activemirror.ai is the identity surface.
- MirrorGate is the policy boundary; MirrorBrain, MirrorRecall, and MirrorBalance are core system layers.
- Use those facts only when relevant. Do not invent metrics, dates, customers, or partnerships."""
...
async def stream_with_judge(messages: list, ip: str, request_id: str, boundary_msg: str = None, routing=None, model_decision=None):
    ...
    if routing and routing.response_type == ResponseType.MIRROR_REFLECT:
...
async def get_knowledge():
    ...
    return {
        ...
        "content": content,
        "last_updated": _iso_mtime(knowledge_path)
    }
...
async def get_capabilities():
    ...
    return {
        ...
        "content": content,
        "last_updated": _iso_mtime(caps_path)
    }
...
async def show_transparency():
    ...
    return {
        ...
        "last_updated": max(
            _iso_mtime(knowledge_path) if knowledge_path.exists() else "",
            _iso_mtime(caps_path) if caps_path.exists() else "",
...
async def mirror(request: Request, body: MirrorRequest):
    ...
    # ═══════════════════════════════════════════════════════════════
    public_reflect_mode = body.systemPrompt is None and (body.mode is None or body.mode == "mirror_only")
    routing_decision = two_lane_router.route(
        body.message,
        user_preference="mirror_only" if public_reflect_mode else body.mode,
    )
    ...
    # ═══════════════════════════════════════════════════════════════
    if public_reflect_mode and model_decision.model.tier == ModelTier.IMAGE_GEN:
        model_decision.model = MODELS[ModelTier.FAST]
        model_decision.reasoning = "Public mirror stays text-first"
        model_decision.query_type = "public_reflection"
        model_decision.confidence = max(model_decision.confidence, 0.9)
    ...
    search_context = None if public_reflect_mode else get_search_context(body.message)
    if search_context:
        base_prompt += f"\n\n{search_context}"
