...
const required = [
    ...
    [mirrorState, 'const SESSION_CONTEXT_MAX_MESSAGES = 4', 'state caps session context at four messages'],
    [mirrorState, 'normalized.sessionContextMessages = normalizeSessionContextMessages', 'tab snapshot normalizes bounded session context'],
    [mirrorState, "['session_context', 'sessionContextTurns', 'sessionContextMessages']", 'durable writes recursively strip ephemeral context fields'],
    ...
    [mirrorState, 'normalizeHomeChatThread(thread)', 'durable chat snapshots omit ephemeral context by default'],
    [mirrorState, "result?.kind === 'privacy_hold'", 'privacy-hold turns do not persist sensitive intent'],
    [mirrorState, 'It survives refresh until the tab closes', 'session state wording matches tab lifetime'],
    ...
    [homePage, 'currentChatSnapshot({ includeSessionContext: true })', 'home page includes prior messages only for tab recovery'],
    ...
    [homePage, 'setSessionContextMessages([])', 'clearing or restoring a durable chat clears ephemeral prior messages'],
