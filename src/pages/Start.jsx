...
function MirrorLogo() {
    return (
        <span className="grid h-9 w-9 place-items-center rounded-lg border border-blue-200/20 bg-white/[0.045] text-cyan-100">
            <Sparkles size={16} />
        </span>
    );
}
...
export default function Start() {
    ...
    return (
        <div className={`min-h-dvh overflow-x-hidden selection:bg-emerald-300/30 ${isLight ? 'bg-[var(--am-canvas)] text-[var(--am-ink)]' : 'bg-[var(--am-canvas)] text-white'}`}>
            ...
            <header className="relative z-10 px-4 py-4">
                <div className="mx-auto flex max-w-5xl items-center justify-between gap-4">
                    <Link to="/" className="inline-flex items-center gap-3">
                        <MirrorLogo />
                        <span className={`text-sm font-semibold ${isLight ? 'text-stone-950' : 'text-white'}`}>Active Mirror</span>
                    </Link>
                    <div className="flex items-center gap-3 text-xs">
                        <Link to="/privacy" className={`hidden transition sm:inline ${isLight ? 'text-stone-600 hover:text-stone-950' : 'text-zinc-400 hover:text-white'}`}>Privacy</Link>
                        <Link to="/terms" className={`hidden transition sm:inline ${isLight ? 'text-stone-600 hover:text-stone-950' : 'text-zinc-400 hover:text-white'}`}>Terms</Link>
                        <button
                            type="button"
                            onClick={toggleTheme}
                            className={`grid h-9 w-9 place-items-center rounded-full border transition ${isLight ? 'border-stone-300/70 bg-white/65 text-stone-600 hover:border-stone-400 hover:text-stone-950' : 'border-white/10 bg-white/[0.035] text-zinc-300 hover:border-cyan-200/30 hover:text-white'}`}
                            aria-label={isLight ? 'Use dark mode' : 'Use light mode'}
                        >
                            {isLight ? <Moon size={15} /> : <Sun size={15} />}
                        </button>
                        <Link to="/" className={`rounded-full border px-4 py-2 font-semibold transition ${isLight ? 'border-stone-300/70 bg-white/65 text-stone-700 hover:border-emerald-400/45 hover:text-stone-950' : 'border-white/10 bg-white/[0.035] text-zinc-300 hover:border-emerald-200/35 hover:text-white'}`}>
                            Chat
                        </Link>
                    </div>
                </div>
            ...
            <main className="relative z-10 mx-auto flex min-h-[calc(100dvh-76px)] w-full max-w-5xl items-center px-4 py-6 sm:py-10">
                {phase === 'scan' ? (
                    <section className="mx-auto grid w-full max-w-4xl gap-5 lg:grid-cols-[0.78fr_1fr] lg:items-center">
                        <div className="px-1 text-center lg:text-left">
                            <h1 className={`mx-auto max-w-lg text-[3.2rem] font-semibold leading-[0.96] tracking-normal sm:text-[4.8rem] lg:mx-0 ${isLight ? 'text-[var(--am-ink)]' : 'text-white'}`}>
                                Set it up.
                            </h1>
                            <p className={`mt-4 text-base font-medium sm:text-lg ${isLight ? 'text-stone-600' : 'text-zinc-400'}`}>
                            ...
                            <div className="mt-7 flex justify-center gap-2 lg:justify-start" aria-label={`Step ${currentStep} of ${SCAN_QUESTIONS.length}`}>
                                {SCAN_QUESTIONS.map((question, index) => {
                                    ...
                                    return (
                                        <span
                                            key={question.id}
                                            className={`h-2.5 rounded-full transition-all ${active ? 'w-9 bg-emerald-200' : done ? 'w-2.5 bg-cyan-200/80' : 'w-2.5 bg-white/14'}`}
                                        />
                                    );
                                })}
                            </div>
                        ...
                        <div className={`rounded-lg border p-5 sm:rounded-lg sm:p-7 ${isLight ? 'border-stone-300/70 bg-white/72 ring-1 ring-white/80' : 'border-white/10 bg-[var(--am-surface)] ring-1 ring-white/[0.04]'}`}>
                            <div className={`mb-5 flex items-center justify-between gap-3 text-xs font-semibold ${isLight ? 'text-stone-600' : 'text-zinc-400'}`}>
                            ...
                            {answers.length > 0 ? (
                                <button
                                    type="button"
                                    onClick={reset}
                                    className={`mt-5 rounded-full border px-4 py-2 text-sm font-semibold transition ${isLight ? 'border-stone-300/70 bg-white/55 text-stone-600 hover:border-stone-400 hover:text-stone-950' : 'border-white/10 bg-white/[0.035] text-zinc-400 hover:border-white/20 hover:text-white'}`}
                                >
                                    Start over
                                </button>
                            ) : null}
                ...
                {phase === 'result' && result ? (
                    <section className="grid w-full gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
                        <div className={`order-2 rounded-lg border p-6 text-center lg:order-1 ${isLight ? 'border-stone-300/70 bg-white/72 ring-1 ring-white/80' : 'border-white/10 bg-[var(--am-surface)] ring-1 ring-white/[0.04]'}`}>
                        ...
                        <div className="order-1 lg:order-2">
                            <h1 className={`max-w-2xl text-[2.85rem] font-semibold leading-[0.98] tracking-normal sm:text-[4.7rem] ${isLight ? 'text-[var(--am-ink)]' : 'text-white'}`}>
                                Ready.
                            </h1>
                            <p className={`mt-5 max-w-xl text-base leading-7 sm:text-lg sm:leading-8 ${isLight ? 'text-stone-600' : 'text-zinc-400'}`}>
                                Start with anything. You can keep a copy if you want.
                            </p>

                            <div className="mt-6 grid gap-2">
                                {result.preferences.slice(0, 4).map((item) => (
                                    <span key={item.id} className={`rounded-lg border px-3 py-2 text-sm font-semibold ${isLight ? 'border-stone-300/70 bg-white/58 text-stone-700' : 'border-white/10 bg-white/[0.05] text-zinc-200'}`}>
                            ...
                            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                                <button
                                    type="button"
                                    onClick={startChat}
                                    className="am-primary-action inline-flex min-h-14 items-center justify-center gap-3 px-6 text-base font-bold"
                                >
                                    Start chat
                                    <ArrowRight size={19} />
                                </button>
                                <button
                                    ...
                                    className={`inline-flex min-h-14 items-center justify-center gap-2 rounded-lg border px-6 text-base font-semibold transition ${isLight ? 'border-stone-300/70 bg-white/60 text-stone-700 hover:border-cyan-500/35 hover:bg-white hover:text-stone-950' : 'border-white/10 bg-white/[0.05] text-zinc-300 hover:border-white/20 hover:text-white'}`}
                                >
                                    <Download size={17} />
                                    Keep a copy
                                </button>
